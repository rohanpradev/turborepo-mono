...
- Use `make k8s-gateway` for local Gateway API routing. It installs the standard CRDs before enabling Traefik's Gateway provider and HTTPS listener, deploys app HTTPRoutes with Ingress disabled, and keeps the browser and monitoring forwards open. Use `make k8s-up-gateway` for deployment only.
